...
def process_file(input_file, output_file):
    with open(input_file, 'r') as infile, open(output_file, 'w') as outfile:
        ...
        for i in range(0, len(lines), 2):
            ...
            for j in range(0, len(sequence) - 2, 15 * 3):  # Move 5 codons forward (5*3 nucleotides)
                segment = sequence[j:j + 20 * 3]  # 20 codons (20*3 nucleotides)
                if len(segment) < 20 * 3:
                    cai_segments.append(calculate_cai(segment))
                    break
                cai_segments.append(calculate_cai(segment))
